...
async def chat_stream(request: ChatRequest):
    ...
    Frontend sends: { query, session_id, stream: true, global_search: bool }
    Conversation history is fetched server-side from MongoDB using session_id.
    """
    async def generate():
        try:
            ...
            # ✅ NEW: Handle global_search flag for streaming
            if not global_search:
                logger.info(f"[STREAM] global_search=False: restricting to uploaded documents only")
                domain_chunks = await orchestrator._retrieve_domain(request.query)

                if not domain_chunks:
                    yield f"data: The requested information is not available in the provided documents.\n\n"
                    yield "data: [DONE]\n\n"
                    return

                # Force to domain intent
                intent_result = Intent.DOMAIN
            ...
            if intent_result.intent == Intent.WEB:
                ...
                from app.config.settings import get_settings
                cfg           = get_settings()
                domain_chunks = await orchestrator._retrieve_domain(intent_result.rewritten_query if hasattr(intent_result, 'rewritten_query') else request.query)
                ...
                if is_confident:
                    ...
                    # If global_search is False and no confident domain chunks, don't fall back to web search
                    if not global_search:
                        async for token in llm_generator.generate_stream(
                            query=request.query,
                            assistant_name=request.assistant_name,
