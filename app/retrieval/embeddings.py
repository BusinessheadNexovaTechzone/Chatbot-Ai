...
class EmbeddingService:
    ...
    async def embed_texts(self, texts: List[str]) -> List[List[float]]:
        if self._use_gemini:
            ...
            # Try per-text cache first (Redis) to avoid repeated CPU encodes
            results: List[List[float]] = [None] * len(texts)
            missing_indices = []
            missing_texts = []
            for i, t in enumerate(texts):
                try:
                    cached = await cache_service.get("embed", t)
                    if cached:
                        results[i] = cached
                        continue
                except Exception:
            ...
            if missing_texts:
                computed = await self._embed_local(missing_texts)
                for idx, vec in zip(missing_indices, computed):
                    results[idx] = vec
                    try:
                        await cache_service.set("embed", texts[idx], vec)
                    except Exception:
                        pass
