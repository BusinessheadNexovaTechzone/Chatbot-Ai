...
# Run with Docker (starts Redis, MongoDB + app)
docker compose up -d
docker compose ps
...
└────────┬────────────┘        └────────────┬─────────────┘
        ...
        ├─ domain  → TurboVec vector search → rerank → Gemini generation
...
| `app/retrieval/vector_store.py` | TurboVec async client (hybrid search) |
...
| `POST` | `/api/v1/ingest` | Scrape website + index into TurboVec |
| `POST` | `/api/v1/upload` | Upload files (PDF/DOCX/XLSX/PPTX) for indexing |
| `GET` | `/v1/health` | Redis, MongoDB, TurboVec status |
...
- **Async-first:** All services use async clients (motor, aioredis).
...
- **Graceful degradation:** Cache miss, MongoDB failure, or TurboVec unavailability are handled independently — the chat still functions.
